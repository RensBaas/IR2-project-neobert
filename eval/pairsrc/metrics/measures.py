"""
Modified from DataResponsibly/FairRank : https://github.com/DataResponsibly/FairRank/blob/master/measures.py
"""
...
NORM_CUTPOINT=1 # cut-off point used in normalizer computation
NORM_ITERATION=20 # max iterations used in normalizer computation
NORM_FILE="src/metrics/normalizer.txt" # externally text file for normalizers
    ...
    for perspective_idx in PERSPECTIVES:
        ...
        for metric_name in METRIC_NAMES:
            ...
            for query_idx in tqdm(retrieved):
                ...
                test_ranking_relevant_subset = [ doc_id for doc_id in test_ranking if ( (doc_id in qrels[query_idx]) and (qrels[query_idx][doc_id]>=qrel_threshold) ) ]
                pro_index = [idx for idx in test_ranking_relevant_subset if f'_p{perspective_idx}_' in idx]
                if not len(pro_index):
                    continue
                user_N = len(test_ranking_relevant_subset)
                pro_N = len(pro_index)

                #if (not normalizer) or (user_N != user_N_) or (pro_N != pro_N_) or (:
                normalizer = getNormalizer(user_N,pro_N,metric_name)

                fairness_score = calculateNDFairness(test_ranking_relevant_subset,
                                                     pro_index,
                                                     _cut_point=CUT_POINT,
                                                     _gf_measure=metric_name,
                                                     _normalizer=normalizer)
                results[metric_name][perspective_idx].append(fairness_score)
    return results
